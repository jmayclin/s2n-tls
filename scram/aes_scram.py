...
def scram_decrypt(K, N, A, C, X, Tag):
    ...
    # T = GMAC (N, A||C, null)
    T_calculated = (
        AES.new(key=KM_calculated, mode=AES.MODE_GCM, nonce=N).update(A + C).digest()
    )
    ...
    PADDING_LEN_STR_calculated = bytes(
        a ^ b for (a, b) in zip(U3_calculated[32:34], X[32:34])
    )
    ...
    # Decrypt Ciphertext
    PADDED_MSG_calculated = AES.new(
        key=KE_calculated, mode=AES.MODE_CTR, nonce=N
    ).decrypt(C)
