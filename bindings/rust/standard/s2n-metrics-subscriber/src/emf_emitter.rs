...
mod tests {
    ...
    // arbitrary numbered policies that won't change. We use two to get
    // a variety of metrics.
    static ARBITRARY_POLICY_1: LazyLock<Policy> =
        LazyLock::new(|| Policy::from_version("20240503").unwrap());
    static ARBITRARY_POLICY_2: LazyLock<Policy> =
    ...
    fn emission() {
        ...
        let server_config = {
            let mut config = config_builder(&ARBITRARY_POLICY_1).unwrap();
    ...
    fn snapshot() {
        ...
        let mut endpoint = TestEndpointWithEmitter::new("cute-kitten", &ARBITRARY_POLICY_1);
