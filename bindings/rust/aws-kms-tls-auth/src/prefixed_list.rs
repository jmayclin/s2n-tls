    ...
    T: DecodeValue,
{
    fn decode_from(buffer: &[u8]) -> std::io::Result<(Self, &[u8])> {
        ...
        let length_usize: usize = match length.try_into() {
            ...
            Err(_) => return Err(std::io::Error::new(ErrorKind::FileTooLarge, "failed to convert to usize")),
        };
...
mod tests {
    ...
    fn list_too_long() {
        ...
        assert_eq!(error.to_string(), "failed to convert list length of 257 into u8");
    }
}