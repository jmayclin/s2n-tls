...
pub struct RustlsConnection {
    // we make the rustls connection host it's own test view, because unlike
    // openssl and s2n-tls, rustls does _not_ hold internal io state. Instead it
    // is passed as a parameter on every read/write call.
    io: ViewIO,
