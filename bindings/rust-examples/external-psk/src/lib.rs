...
use s2n_tls::callbacks::{OfferedPskListRef, PskSelectionCallback};
use s2n_tls::{
...
impl PskStore {
    ...
    pub fn get(&self, identity: u64) -> Option<Psk> {
        self.keys.get(&identity).map(|key| {
            let mut builder = Psk::builder().unwrap();
            builder.set_identity(&identity.to_ne_bytes()).unwrap();
            builder.set_secret(key).unwrap();
            builder.set_hmac(s2n_tls::enums::PskHmac::SHA384).unwrap();
...
impl PskSelectionCallback for PskStore {
    fn select_psk(&self, connection: &mut Connection, psk_list: &mut OfferedPskListRef) {
        ...
        tracing::debug!("selecting the psk");
        while let Some(id_result) = identities.next() {
            // "parse" the wire input to the expected u64 identity
            let id = match id_result {
                // DANGER: id_slice is untrusted wire input. Customers should take
                // care not to `panic` or `unwrap` based on this input.
                Ok(id_slice) => match id_slice[0..8].try_into() {
                    Ok(id_array) => u64::from_ne_bytes(id_array),
                    Err(_) => {
                        tracing::warn!("client presented malformed PSK: {:?}", id_slice);
                        continue;
                    }
                }
                Err(e) => {
                    tracing::error!("Failed to retrieve PSK ID from s2n-tls: {e}");
                    // this is an exceptional failure case, we do not expect future
                    // calls to succeed
                    return;
                }
            };

            // The identity has a recognized format, see if we have the corresponding
            // secret.
            match self.get(id) {
                Some(matched_psk) => {
                    connection.append_psk(&matched_psk).unwrap();
                    tracing::info!("negotiating with PSK identity {:?}", id);
                    identities.choose_current_psk().unwrap();
                    return;
                },
                None => {
                    tracing::warn!("client presented unrecognized PSK: {:?}", id);
                }
            }
        }

        tracing::warn!("no acceptable PSKs found, failing handshake");
    }
}
...
impl From<Psk> for ClientPsk {
    fn from(value: Psk) -> Self {
        ClientPsk { psk: value }
    }
}
...
mod scenarios {
    ...
    async fn few_keys_example() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let client_3_psk = {
            ...
            builder.set_identity(b"not a known psk")?;
            builder.set_secret(b"123456928374928734123123")?;
            builder.set_hmac(s2n_tls::enums::PskHmac::SHA384)?;
            builder.build()
        }
        .unwrap()
        .into();
