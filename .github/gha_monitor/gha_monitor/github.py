...
class GitHubClient:
    ...
    def __init__(self):
        self._github = GitHub.GitHub(
            ...
            password=self.params["github_password"],
    ...
    def get_workflow_log_chunk(self, chunk=1, final_state="failure"):
        ...
        """
        (status_code, self.response) = self._github.repos[self.repo_org][
            self.repo
        ].actions.runs.get(page=chunk, status=final_state)
        return status_code

    def get_workflow_name(self, workflow_id):
        logging.debug(f"Looking up workflow_id {workflow_id}")
        (status_code, response) = (
            self._github.repos[self.repo_org][self.repo]
