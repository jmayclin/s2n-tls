...
TIME_WINDOW_BEGIN = datetime.now().astimezone(tz.UTC) - timedelta(
    hours=float(os.getenv("MONITOR_FREQ_IN_HOURS"))
)
...
class S2nNotices(sns.SNSClient):
    params = {"topic_arn": "arn:aws:sns:us-west-2:024603541914:s2n_notices"}
...
def main():
    ...
    logging.info(
        f"Looking for failures newer than {TIME_WINDOW_BEGIN} in {gh_api.params['repo_organization']}"
    ...
    if gh_api.worklog:
        for enhanced_worklog in gh_api.worklog:
            ...
            if datetime_creation > TIME_WINDOW_BEGIN:
                ...
                enhanced_worklog["workflow_name"] = gh_api.get_workflow_name(
                    enhanced_worklog["workflow_url"].split("/")[-1:][0]
                )
    ...
    if plaintext_notice:
        ...
        logging.info("Notices published")
    logging.info("Done")
