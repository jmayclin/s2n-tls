...
def pytest_addoption(parser):
    ...
    parser.addoption(
        ...
        default=False,
        type=int,
        help="S2N is running in FIPS mode",
    )
    parser.addoption(
        "--no-pq",
        action="store",
        ...
        default=False,
        type=int,
        help="Turn off PQ support",
    )
    parser.addoption(
        "--provider-criterion",
        action="store",
        dest="provider-criterion",
        default="off",
        type=str,
        choices=["off", "baseline", "delta"],
        help="Use Criterion provider in one of 3 modes: [off,baseline,delta]",
    )
