...
use tls_shim_interop::ServerTls;
...
fn get_config(test: InteropTest, cert_pem: &[u8], key_pem: &[u8]) -> Option<Result<s2n_tls::config::Config, Box<dyn Error>>> {
    let mut config = Config::builder();
    config.set_security_policy(&DEFAULT_TLS13)?;
    config.load_pem(cert_pem, key_pem)?;
    Some(Ok(config.build()?))
}
...
async fn run_server<Tls: ServerTls>(config: Tls::Config, port: u16, test: InteropTest) -> Result<(), Box<dyn Error>> {
    ...
    loop {
        ...
        tokio::spawn(async move {
            let mut tls = Tls::accept(&server, stream).await?;
